...
Eigen::Vector3d safeRegionRrtStarEllip::computeNoiseStd(const Eigen::Vector3d& point) {
    ...
    double sigma_x = (0.001063 + 0.0007278 * x + 0.003949 * x * x)*sqrt(11.35);

    // Compute lateral noise (σy, σz)
    double sigma_y = 0.04*sqrt(11.35);
    double sigma_z = 0.04*sqrt(11.35);
...
void safeRegionRrtStarEllip::reset()
{     
    treeDestruct();
...
void safeRegionRrtStarEllip::setStartPt( Vector3d startPt, Vector3d endPt)
{
    start_pt = startPt;
    end_pt   = endPt; 
...
void safeRegionRrtStarEllip::setPt( Vector3d startPt, Vector3d endPt, double xl, double xh, double yl, double yh, double zl, double zh,
...
void safeRegionRrtStarEllip::setInput(pcl::PointCloud<pcl::PointXYZ> cloud_in, Eigen::Vector3d origin)
{     
...
void safeRegionRrtStarEllip::setInputforCollision(pcl::PointCloud<pcl::PointXYZ> cloud_in)
{
    kdtreeForCollisionPred.setInputCloud(cloud_in.makeShared());
}

inline double safeRegionRrtStarEllip::getDis(const NodePtr node1, const NodePtr node2){
      return sqrt(pow(node1->coord(0) - node2->coord(0), 2) + pow(node1->coord(1) - node2->coord(1), 2) + pow(node1->coord(2) - node2->coord(2), 2) );
}

inline double safeRegionRrtStarEllip::getDis(const NodePtr node1, const Vector3d & pt){
      return sqrt(pow(node1->coord(0) - pt(0), 2) + pow(node1->coord(1) - pt(1), 2) + pow(node1->coord(2) - pt(2), 2) );
}

inline double safeRegionRrtStarEllip::getDis(const Vector3d & p1, const Vector3d & p2){
      return sqrt(pow(p1(0) - p2(0), 2) + pow(p1(1) - p2(1), 2) + pow(p1(2) - p2(2), 2) );
}

// inline double safeRegionRrtStarEllip::radiusSearch( Vector3d & search_Pt)
// {     
...
//     geometry_utils::Ellipsoid obs_ellip = geometry_utils::Ellipsoid(Eigen::Matrix3f::Identity(), std_dev_vector, obs);
...
inline double safeRegionRrtStarEllip::radiusSearch(Vector3d & search_Pt)
{     
    // Convert search point to PCL format
    pcl::PointXYZ searchPoint;
    searchPoint.x = search_Pt(0);
    searchPoint.y = search_Pt(1);
    searchPoint.z = search_Pt(2);
    ...
    if (distance < 1e-6)  // Avoid division by zero
        return min(float(max_radius), sqrt(pointRadiusSquaredDistance[0]));
...
inline double safeRegionRrtStarEllip::radiusSearchCollisionPred( Vector3d & search_Pt)
{     
...
void safeRegionRrtStarEllip::clearBranchS(NodePtr node_delete) // Strong branch cut: no matter how, cut all nodes in this branch
{     
    for( auto nodeptr: node_delete->nxtNode_ptr ){
        if( nodeptr->valid)
            invalidSet.push_back(nodeptr);
...
void safeRegionRrtStarEllip::treePrune(NodePtr newPtr)
{     
    NodePtr ptr = newPtr;
...
void safeRegionRrtStarEllip::removeInvalid()
{     
...
Eigen::Vector3d safeRegionRrtStarEllip::getRootCoords()
{
    return root_node->coord;
}

void safeRegionRrtStarEllip::resetRoot(Vector3d & target_coord)
{   
...
void safeRegionRrtStarEllip::solutionUpdate(double cost_reduction, Vector3d target)
{
    for(auto nodeptr: NodeList){
...
void safeRegionRrtStarEllip::updateHeuristicRegion(NodePtr update_end_node)
{   
...
inline Vector3d safeRegionRrtStarEllip::genSample()
{     
    Vector3d pt;
...
bool safeRegionRrtStarEllip::isInFOV(const Eigen::Vector3d& pt)
{
...
inline NodePtr safeRegionRrtStarEllip::genNewNode( Vector3d & pt_sample, NodePtr node_nearst_ptr )
{
...
bool safeRegionRrtStarEllip::checkTrajPtCol(Vector3d & pt)
{     
    if(radiusSearchCollisionPred(pt) < 0 ) return true;
    else return false;
}

inline bool safeRegionRrtStarEllip::checkEnd( NodePtr ptr )
{    
...
inline NodePtr safeRegionRrtStarEllip::findNearstVertex( Vector3d & pt )
{
...
inline int safeRegionRrtStarEllip::checkNodeRelation( double dis, NodePtr node_1, NodePtr node_2 )
{
...
#if 1
void safeRegionRrtStarEllip::treeRewire( NodePtr node_new_ptr, NodePtr node_nearst_ptr)
{     
...
void safeRegionRrtStarEllip::recordNode(NodePtr new_node)
{
    NodeList.push_back(new_node);
}

void safeRegionRrtStarEllip::tracePath()
{   
...
void safeRegionRrtStarEllip::treeDestruct()
{
    if(kdTree_ != NULL)
    {
        kd_free(kdTree_);
...
inline double safeRegionRrtStarEllip::checkRadius(Vector3d & pt)
{
    return radiusSearch( pt );
}

inline int safeRegionRrtStarEllip::checkNodeUpdate(double new_radius, double old_radius)
{
...
inline bool safeRegionRrtStarEllip::isSuccessor(NodePtr curPtr, NodePtr nearPtr) // check if curPtr is father of nearPtr
{     
...
inline bool safeRegionRrtStarEllip::checkValidEnd(NodePtr endPtr)
{
    NodePtr ptr = endPtr;
...
void safeRegionRrtStarEllip::SafeRegionExpansion( double time_limit )
{   
...
void safeRegionRrtStarEllip::SafeRegionRefine( double time_limit )
{   
...
void safeRegionRrtStarEllip::SafeRegionEvaluate( double time_limit )
{   
...
void safeRegionRrtStarEllip::treeRepair(double time_limit, vector< pair<Vector3d, double> > & node_list)
{   
