...
namespace super_planner {
    ...
    double CIRI_s::computeThetaY(const Eigen::Vector3d& point) {
    ...
    Eigen::Vector3d CIRI_s::computeNoiseStd(const Eigen::Vector3d& point) {
    ...
    Eigen::Matrix3Xd CIRI_s::computePointCloudNoise(const Eigen::Matrix3Xd& pointCloud, const Eigen::Vector3d &o) {
    ...
    RET_CODE CIRI_s::convexDecomposition(const Eigen::MatrixX4d& bd, const Eigen::Matrix3Xd& pc, const Eigen::Vector3d& a,
                                       const Eigen::Vector3d& b, const Eigen::Vector3d &o, std::vector<Ellipsoid> &tangent_obs) {
...
//        bool infeasible_problem{false};
        ...
        for (int loop = 0; loop < iter_num_; ++loop) {
            ...
            for (int i = 0; !completed && i < (M + N); ++i) 
            {
                if (minSqrD < minSqrR) {
                    ...
                    if(dis < robot_r_ - 1e-2) {
                        ...
                        cout<<YELLOW<<" -- [CIRI_s] WARNING! The problem is not feasible, the min dis to obstacle is only: "<<dis<<RESET<<endl;
                        return FAILED;
                        cout<<" -- [CIRI_s] dis: "<<dis<<endl;
                        cout<<" -- [CIRI_s] robot_r: "<<robot_r_<<endl;
                        cout<<" -- [CIRI_s] pcMin: "<<pt_w.transpose()<<endl;
                        cout<<" -- [CIRI_s] a: "<<a.transpose()<<endl;
                        cout<<" -- [CIRI_s] b: "<<b.transpose()<<endl;
                    }
            ...
            if(hPoly.array().isNaN().any()) {
                cout << YELLOW << " -- [CIRI_s] ERROR! maxVolInsEllipsoid failed." << RESET << endl;
                return FAILED;
            }
        ...
        if (std::isnan(hPoly.sum())) {
            cout << YELLOW << " -- [CIRI_s] ERROR! There is nan in generated planes." << RESET << endl;
        ...
        if (!geometry_utils::findInterior(hPoly, inner)) {
            cout<<RED<<" -- [CIRI_s] The polytope is empty."<<RESET<<endl; 
            return FAILED;
        }
    ...
    void CIRI_s::getPolytope(Polytope& optimized_poly) {
        optimized_poly = optimized_polytope_;
    }

    void CIRI_s::setupParams(double robot_r, int iter_num) {
        robot_r_ = robot_r;
        iter_num_ = iter_num;
    ...
    void CIRI_s::findTangentPlaneOfSphere(const Eigen::Vector3d& center, const double& r,
        ...
        //     cout<<YELLOW<<" -- [CIRI_s] findTangentPlaneOfSphere: The pass point is inside the sphere."<<RESET<<endl;
        //     return;
        // }
        //
    ...
    void CIRI_s::findEllipsoid(const Eigen::Matrix3Xd& pc,
        ...
        if (max_iter == 0) {
            cout << YELLOW << " -- [CIRI_s] Find Ellipsoid reach max iteration, may cause error." << endl;
        }
        max_iter = 100;
    ...
    void CIRI_s::findTangentPlaneOfEllipsoid(const Ellipsoid obstacle, 
                                        const Eigen::Vector3d& pass_point,
                                        const Eigen::Vector3d& seed_p,
                                        Eigen::Vector4d& outer_plane)
    {
        std::cout<<"[CIRI_s debug] plane originally: "<<outer_plane<<std::endl;
        ...
        std::cout<<"[CIRI_s debug] plane before transformation: "<<outer_plane<<std::endl;
        outer_plane = obstacle.toWorldFrame(outer_plane);
        std::cout<<"[CIRI_s debug] plane after transformation: "<<outer_plane<<std::endl;

    }

    double CIRI_s::checkTangency(const Ellipsoid& E, const Eigen::Vector4d& plane) {
